...
use JayGaha\LumenFormRequest\Tests\TestCase;
use JayGaha\LumenFormRequest\Requests\FormRequest;
use JayGaha\LumenFormRequest\Requests\BaseRequest;
use JayGaha\LumenFormRequest\Requests\FormRequestServiceProvider;
...
expect()->extend('toHaveValidationError', function (string $field) {
    $errors = $this->value->errors()->messages();

    return expect($errors)->toHaveKey($field);
});

expect()->extend('toHaveValidationErrors', function (array $fields) {
    $errors = $this->value->errors()->messages();
